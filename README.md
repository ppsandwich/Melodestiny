...
- **AI Lyric Generator (Best-of-Three):** Paste your OpenRouter API Key and select your preferred model (Claude, Gemini, Llama, GPT) via a header popover. Enter a prompt to trigger three parallel generation streams; the client-side WebAssembly engine immediately grades each variation and loads the highest-scoring candidate.
